...
- `src/components/games/` — the five game types kids actually play: `MultipleChoice`, `FillInBlank`, `FlashcardMatch`, `WordImageMatch`, `AudioRiddle`, `MysteryMission` (riddle-guessing). `GameShell.tsx` is the shared wrapper (scoring, transitions between questions).
