...
- **Added a favicon**: `src/app/icon.tsx` generates a 32×32 gradient 🌟 icon via Next's built-in icon convention. Verified the `<link rel="icon">` tag is correctly injected and the route builds as a static asset. (A direct `/favicon.ico` probe some browsers make as a secondary fallback still 404s — harmless and doesn't affect the actual tab icon, which now works via the `<link>` tag; could add a literal `favicon.ico` too later if we want to silence that specific request.)
...
2. Pick the first Horizon 2 feature to prototype.
